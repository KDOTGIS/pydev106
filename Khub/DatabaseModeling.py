...
from arcpy import ExportXMLWorkspaceDocument_management, env
...
def EnterpriseModel():    
    ...
    rhpass = raw_input('RH password')
    DBName = "KH04a"
    CreateEnterpriseGeodatabase_management("SQL_Server", r"sqlgis\gdb_dev", DBName, "DATABASE_AUTH", "geo_admin", sapass, "SDE_SCHEMA", "sde", sdepass, "", "C:/gisdata/keycodes/License10.6/sysgen/keycodes")
    CreateDatabaseConnection_management(r"C:/gisdata/DBConnections/Khub/KHDev", "sde@"+DBName, "SQL_Server", r"sqlgis\gdb_dev", "DATABASE_AUTH", "sde", sdepass, "SAVE_USERNAME", DBName)
    CreateDatabaseConnection_management(r"C:/gisdata/DBConnections/Khub/KHDev", "sa@"+DBName, "SQL_Server", r"sqlgis\gdb_dev", "DATABASE_AUTH", "geo_admin", sapass, "SAVE_USERNAME", DBName)
    CreateDatabaseUser_management(r"C:/gisdata/DBConnections/Khub/KHDev/sa@"+DBName+".sde",  "DATABASE_USER", "rh", rhpass)
    CreateDatabaseConnection_management(r"C:/gisdata/DBConnections/Khub/KHDev", "rh@"+DBName, "SQL_Server", r"sqlgis\gdb_dev", "DATABASE_AUTH", "rh", rhpass, "SAVE_USERNAME", DBName)
...
def ValidatetoFileGDB():
    ...
    xmlIn = r"C:\temp\KHDev\KH04B.xml"
    gdbname = "KH04B"
    GDB_In = r"C:/temp/KHDev/"+gdbname
    gdbin = GDB_In+".gdb"
    if Exists(gdbin):
        Delete_management(gdbin, "Workspace")
        CreateFileGDB_management(r"C:/temp/KHDev", gdbname, "CURRENT")
...
def EnterpriseModelTest():    
    from arcpy import CreateEnterpriseGeodatabase_management, CreateDatabaseConnection_management, CreateDatabaseUser_management, ImportXMLWorkspaceDocument_management, Rename_management
    from getpass import getpass as getPassword
    sapass = raw_input('sysadmin password')
    sdepass = raw_input('sde password')
    rhpass = raw_input('RH password')
    DBName = "KH04B"
    xmlIn = r"C:/temp/KHDev/"+DBName+".xml"     
    ...
    CreateDatabaseConnection_management(r"C:/gisdata/DBConnections/Khub/KHDev", "sde@"+DBName, "SQL_Server", r"sqlgis\gdb_dev", "DATABASE_AUTH", "sde", sdepass, "SAVE_USERNAME", DBName)
    CreateDatabaseConnection_management(r"C:/gisdata/DBConnections/Khub/KHDev", "sa@"+DBName, "SQL_Server", r"sqlgis\gdb_dev", "DATABASE_AUTH", "geo_admin", sapass, "SAVE_USERNAME", DBName)
#   CreateDatabaseUser_management("C:/gisdata/DBConnections/Khub/KHDev/sa@KH04Atest.sde", "DATABASE_USER", "rh", user_password="*****", "#", tablespace_name="")
    CreateDatabaseUser_management(r"C:/gisdata/DBConnections/Khub/KHDev/sa@"+DBName+".sde",  "DATABASE_USER", "rh", rhpass, "#", tablespace_name="")
    CreateDatabaseConnection_management(r"C:/gisdata/DBConnections/Khub/KHDev", "rh@"+DBName, "SQL_Server", r"sqlgis\gdb_dev", "DATABASE_AUTH", "rh", rhpass, "SAVE_USERNAME", DBName)
    gdbin = r"C:/gisdata/DBConnections/Khub/KHDev/rh@"+DBName+".sde"
    ImportXMLWorkspaceDocument_management(gdbin, xmlIn, "SCHEMA_ONLY")

def RenameTestSchemaXML():
 
    from arcpy import Rename_management, Exists
    DBName = "KH04Atest"
    renamestring = "KH04a_RH_"
    TargetDB = r"C:/gisdata/DBConnections/Khub/KHDev/rh@"+DBName+".sde"
    ...
    for network in NetworksList:
        ...
        if Exists(TargetDB+r"/"+DBName+".RH."+network):
            ...
            print(network)
            Rename_management(TargetDB+r"/"+DBName+".RH."+renamestring+network, TargetDB+r"/"+DBName+".RH."+network)
            #ImportXMLWorkspaceDocument_management(TargetDB, r"C:/temp/KHDEV/"+network+".xml", "SCHEMA_ONLY")
    ...
    for event in EventList:
        ...
        if Exists(TargetDB+r"/"+DBName+".RH."+importfeature):
            ...
            Rename_management(TargetDB+r"/"+DBName+".RH."+renamestring+event, TargetDB+r"/"+DBName+".RH."+event)
            #ImportXMLWorkspaceDocument_management(TargetDB, r"C:/temp/KHDEV/"+importfeature+".xml", "SCHEMA_ONLY")
    

def main():
    #exportDevSchemaXML()  ## First run this to get updated XML Files for each feature or table in the geodatabase
    #EnterpriseModel()  ## run this to create the database and connections
    #importDevSchemaXML()  ## Run this to import the XML files into a combined, solid Geodatabase
    #exportSubsetDevSchemaXML()  ## run this to export the combined Geodatabase schema to a valid ArcGIS XML workspace for design in Sparx
    #DeleteAbstractFields()  ## run this if you desire to remove these event fields to model them as abstract fields in Sparx
    #ValidatetoFileGDB()   ## run this to quickly test if the XML workspace will import to a file geodatabase
    EnterpriseModelTest()  ## run this to create the database and connections for the design XML database, and import the modeled geodatabase
    #RenameTestSchemaXML() ## run this to rename the features, which may retain the undesired SCHEMA and OWNER from the Sparx model
    #StageNewEvent()
    #AppendNewEventData()
    #StageAndRegister()
